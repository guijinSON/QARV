...
# Call the function
output = generate_cot(args.model_path, df, args.last_option, args.options, args.bs, args.device)
save_path = args.model_path.split('/')[1]
pd.DataFrame(output).to_csv(f'results/{save_path}-{args.language}-{args.iteration}-cot.csv',index=False)
